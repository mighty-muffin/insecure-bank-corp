...
[prek](https://github.com/j178/prek): an alternative to `pre-commit`, re-engineered in Rust.
...
> **_NOTE:_** Pre-commit hooks run all checks automatically when enabled properly.
...
> **_NOTE:_** `ruff` is not fully implemented yet; do not run it.
...
> **_NOTE:_** `ty` is not fully implemented yet; do not run it.
